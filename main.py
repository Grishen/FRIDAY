...
def process_command(query: str, voice_raw: Optional[str] = None) -> None:
    ...
    from voice_router import brain_first_enabled, invoke_agent_brain, try_fast_path

    try:
        if try_fast_path(query, speak=speak, speak_reply=speak_reply):
            return
    except JarvisExitRequest:
        raise

    _brain_ready = jb is not None and jb.is_brain_enabled()
    if brain_first_enabled() and _brain_ready:
        invoke_agent_brain(
            query,
            voice_raw,
            speak=speak,
    ...
    # Legacy regex ladder — used when brain-first is off, or brain unavailable.
    ...
    # 15) help — legacy fallback when brain-first is off
    elif 'help' in query:
        from voice_router import help_text
    ...
    # 18) exit — legacy fallback when brain-first is off
    elif 'exit' in query:
        ...
        if _brain_ready:
            ...
            speak(
                "Sir, nothing matched and no reasoning engine is available. "
