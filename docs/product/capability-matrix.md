...
| Voice | Wake word — Porcupine WASM | **Done (prototype)** — `NEXT_PUBLIC_PICOVOICE_ACCESS_KEY` · built-in phrases via **`NEXT_PUBLIC_WAKE_BUILTIN`** (`apps/web`). Custom “Friday…” requires Picovoice custom `.ppn` + Picovoice training (not bundled). |
...
| Personal OS | Streaming LLM + sandboxed host tools | **In progress** — orchestration over WebSocket **`assistant.delta`** + SSE; **OpenAI Realtime WebRTC** (`/realtime/webrtc`, `RealtimeDuplexPanel`); **Porcupine** wake; **`local.*`** host tools; browser TTS/Mic/Whisper uploads in **`apps/web`**. |
